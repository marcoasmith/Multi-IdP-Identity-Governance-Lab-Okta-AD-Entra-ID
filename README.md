...
| Okta (Developer Edition) | Cloud IdP for SaaS SSO and access governance |
...
```
                   ...
                   │  Access Certification │  │  Log Analytics            │
                   │  System Log (API)     │  │  Sign-in Logs (Graph API) │
                   └─────────────────────┘  └──────────────────────────┘
```
...
- Ran a simulated access certification review in Okta against an application group
- Simulated an orphaned account scenario by disabling a user in AD and observing propagation delay before deprovisioning completed downstream
- Built a script using the Okta API and Microsoft Graph API to cross-reference active accounts in Okta and Entra ID against AD status and flag mismatches
...
- Access certification and identity governance review processes
- Cross-system reporting using the Okta API and Microsoft Graph API
